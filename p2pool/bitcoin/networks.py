...
nets = dict(
    ...
    defcoin=math.Object(
        ...
        RPC_PORT=39337,
        RPC_CHECK=defer.inlineCallbacks(lambda bitcoind: defer.returnValue(
            'defcoinaddress' in (yield bitcoind.rpc_help()) and
            not (yield bitcoind.rpc_getinfo())['testnet']
        )),
        SUBSIDY_FUNC=lambda height: 50*100000000 >> (height + 1)//840000,
        POW_FUNC=lambda data: pack.IntType(256).unpack(__import__('ltc_scrypt').getPoWHash(data)),
        BLOCK_PERIOD=2*60, # s
        SYMBOL='DFC',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'defcoin') if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/defcoin/') if platform.system() == 'Darwin' else os.path.expanduser('~/.defcoin'), 'defcoin.conf'),
        BLOCK_EXPLORER_URL_PREFIX='http://defcoin.assmeow.org/block/',
        ADDRESS_EXPLORER_URL_PREFIX='http://defcoin.assmeow.org/address/',
        TX_EXPLORER_URL_PREFIX='http://defcoin.assmeow.org/tx/',
        SANE_TARGET_RANGE=(2**256//1000000000 - 1, 2**256//1000 - 1),
        DUMB_SCRYPT_DIFF=2**16,
        DUST_THRESHOLD=0.03e8,
    ),
